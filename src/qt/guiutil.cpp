...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("folgorycoin"))
        return false;
...
{
    // Convert folgorycoin:// to folgorycoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("folgorycoin://"))
    {
        uri.replace(0, 12, "folgorycoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FolgoryCoin.lnk";
}
...
{
    return GetAutostartDir() / "folgorycoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FolgoryCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  folgorycoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("FolgoryCoin-Qt"));
